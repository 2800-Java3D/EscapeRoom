...
public class SoundUtilityJOAL {
    ...
    private int[] initBuffer(String nm) {
        ...
        int[] format = new int[1], size = new int[100], freq = new int[50], loop = new int[1];

        String fnm = SOUND_DIR + nm + ".wav";
        try { // load WAV file into data array first
            ALut.alutLoadWAVFile(fnm, loop, data, loop, loop, loop);
        } catch (ALException e) {
    ...
    /*
     * function to use the sound buffer to initialize a sound source at (0,0,0) with
     * no velocity. he sound may play looping, depending on 'toLoop'.
     */
    ...
    /*
     * function to position the listener at (xNew,zNew) (y is always 0, on the
     * floor)
     */
